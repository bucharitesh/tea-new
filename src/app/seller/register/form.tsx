...
const sellerSchema = z.object({
  ...
  fssaiNo: z.string().optional().or(z.literal("")),
  tmcoNo: z.string().optional().or(z.literal("")),
  bankAccountNo: z
    .string()
    .min(9, "Bank account number must be at least 9 digits"),
  ifscCode: z.string().regex(/^[A-Z]{4}0[A-Z0-9]{6}$/, "Invalid IFSC code"),
  nameOfTransport: z.string().optional().or(z.literal("")),
});
...
export function SellerRegistrationForm() {
  ...
  async function onSubmit(values: z.infer<typeof sellerSchema>) {
    try {
      const res: any = await fetch("/api/register/seller", {
        method: "POST",
        headers: {
          ...
          <CardContent className="grid gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>FSSAI Number (Optional)</FormLabel>
                    <FormControl>
                      <Input {...field} />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>TMCO Number (Optional)</FormLabel>
                    <FormControl>
                      <Input {...field} />
                    </FormControl>
                    <FormMessage />
        ...
        <Card>
          ...
          <CardContent>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Name of Transport (Optional)</FormLabel>
                  <FormControl>
                    <Input {...field} />
                  </FormControl>
                  <FormMessage />
